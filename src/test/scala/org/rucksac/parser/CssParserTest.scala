...
class CssParserTest {
  ...
  @Test
  def testParseCss18() {
    val result = TestCssParser.testParse("input:checked:first-child")
    Assert.assertEquals("*|input:checked:first-child", result.toString)
  }

  @Test
  def testParseCss19() {
    val result = TestCssParser.testParse(":checkbox")
  ...
  def testParseCss20() {
    ...
    val result = TestCssParser.testParse("p::first-line span::first-letter")
    Assert.assertEquals("*|p:first-line *|span:first-letter", result.toString)
  }

  @Test
  def testParseCss21() {
    val result = TestCssParser.testParse("#bar:not(div)")
    Assert.assertEquals("*|*#bar:not(*|div)", result.toString)
  }

  @Test
  def testParseCss22() {
    val result = TestCssParser.testParse("html|*:not(:link):not(:visited)")
    Assert.assertEquals("html|*:not(:link):not(:visited)", result.toString)
  }

  @Test
  def testParseCss23() {
    val result = TestCssParser.testParse("html|tr:nth-child(-n+6)")
    Assert.assertEquals("html|tr:nth-child(- n + 6)", result.toString)
  }

  @Test
  def testParseCss24() {
    val result = TestCssParser.testParse(":nth-child( +3n - 2 )")
    Assert.assertEquals("*|*:nth-child(+ 3n - 2)", result.toString)
  }

  @Test
  def testParseCss25() {
    val result = TestCssParser.testParse(":lang(fr-be) > q")
    Assert.assertEquals("*|*:lang(fr-be) > *|q", result.toString)
  }

  @Test
  def testParseCss26() {
    val result = TestCssParser.testParse(":not(:notX(x))")
